*******************************************************************************
*
* Concurrent Technologies Linux Enhanced 64 bit VME Device Driver Package
*
...
RHEL 5.3 64 bit kernel. The package also contains all the necessary files to allow the
...
readme_64bit.txt           - this file containing installation instructions.
history_64bit.txt          - version history information.
linuxenhancedvme_rev0X - linux enhanced VME device driver user guide
linuxvmeen-xx.x86_64.rpm 	   - a rpm file containing the software where xx is 
			     the version number.
...
rpm -iv linuxvmeen-xx.x86_64.rpm    where xx is the version number
...
A configured Linux kernel source tree, a version 2.6 kernel is required. 
...
Universe II supports only 32 bit addresses.
Only the memory addresses mapped in the 32 bit address space
can be used for the VME slave images and DMA .In case of a board having >= 4GB fitted
on it, some part of the physical memory will be relocated above the 32 bit 
address space rendering it unusable.The amount of memory relocated to this address 
space depends upon the size of the PCI address window configured in the BIOS.As the 
PCI address window goes on increasing, the memory relocated to the 64 bit address space
goes on increasing.Using the mem= command line parameter we need to reduce the
memory so that the top of the memory is in the 32 bit address space along with 
the reserved memory which can then be used by for the Universe II for the VME
slave images and the DMA. 

Eg: In case of a 4GB board and 128MB PCI window, the amount of physical memory
mapped in the 32 bit address space is 3327MB(This value can be obtained by
reading the e820 map dumped in the dmesg.Hence if we want to reserve 8MB
of reserved memory, we would need to set the mem= command line parameter to
3319M. 
