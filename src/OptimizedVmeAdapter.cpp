...
void OptimizedVmeAdapter::readInterruptStatus(UINT32* status) throw(std::string){
	...
	//Reset Interrupt
	write32(Singleton<ConfigFile, CreateThreadSafe>::instance()->getParameter<UINT32>("VME_INTERRUPT_RESET", hexadecimal),0);
}
...
void OptimizedVmeAdapter::interruptCallback(void *arg){
	...
	//Reset Interrupt
	pAdapter->write32(Singleton<ConfigFile, CreateThreadSafe>::instance()->getParameter<UINT32>("VME_INTERRUPT_RESET", hexadecimal),0);
